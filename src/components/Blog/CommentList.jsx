...
export default class CommentList extends Component {
  ...
  handlePraiseComment(id) { // 点赞
    const currentNode = this.refs[`praiseCount${id}`];
    const count = currentNode.innerText;
    const isPraise = currentNode.getAttribute('data-praise') === 'true';
    const cid = currentNode.getAttribute('data-id');// 评论ID
    requiredLogin(() => $.post('/Social/Blog/PraiseComment', { id: cid }, () => {
      const oldCount = currentNode.innerText;
  ...
  render() {
    const commentItems = this.props.itemList.map((itemComment, i) => { // 评论列表项
      ...
      if (itemComment.Comment.ParentId > 0) {
        replayContent = (
          <span>
            ...
            <span dangerouslySetInnerHTML={{ __html: itemComment.Comment.CommentsBody }} />
          </span>
                );
      } else {
        replayContent = (
