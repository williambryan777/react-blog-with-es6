...
export default class BlogButton extends Component {
  static propTypes= {
    ...
    BlogDetail: React.PropTypes.string.isRequired,
  }
  ...
  handleCollection() { // 收藏操作
    const state = this.state.isFavorite;

    requiredLogin(() => $.post('/Social/Blog/CollectMicroBlog', { id: this.props.Id }, () => {
  ...
  handleMove() {
    const currentNode = $(`#replayCount_${this.props.Id}`)[0];
    const parent = $(currentNode).parent()[0];
    const offset1 = parent.offsetTop;
    const offset2 = parent.offsetParent.offsetTop;
    let offset = offset1 + offset2 - 230;
  ...
  handlePageComment(page) {
     ...
     */
  handleChangeComment() {
    console.log('子组件添加评论了。。。');
        // let count=this.state.replyCount+1;
    this.setState({
  ...
  render() {
    const { Id, IsFavorites, IsPraise, ForwardedCount, ReplyCount, PraiseCount, ReportTimeStr } = this.props;

    const cssPraise = this.state.isPraise ? 'handle handle-zan_a' : 'handle handle-zan';
    return (
      <div>
        <div className="feed_list_options">
          <ul className="feed_list clearfix">
            ...
            <li>
              <p className="fl vertical-line-h1 vertical-line" />
            </li>
            ...
            <a href="javascript:void(0);">
              <li onClick={this.handleCommit.bind(this)}>评论(<span id={`replayCount_${this.props.Id}`}>
                {this.state.replyCount}</span>)
                            </li>
            </a>
            <li>
              <p className="fl vertical-line-h1 vertical-line" />
          ...
          <a style={{ cursor: 'pointer' }} href={this.props.BlogDetail} target="_blank"> {ReportTimeStr}</a>
        </div>
        <CommentBox id={this.props.Id} itemList={this.state.cList} rowCount={this.state.rowCount} changeComment={this.handleChangeComment.bind(this)} />
      </div>
    );
  }
}
