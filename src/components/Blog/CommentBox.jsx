...
export default class CommentBox extends Component {
  static propTypes={
    ...
    changeComment: React.PropTypes.func,
  }
  ...
  constructor(props) {
    ...
    /** 父亲组件评论更新回调 */
  handleChangeComment() {
    if (typeof this.props.changeComment === 'function') {
  ...
  handleAddComment(event) { // 提交评论
    ...
    const combody = this.refs.commentInput.value;// 评论内容
    if (!(combody && Boolean($.trim(combody)))) {
      msg('请输入有效内容', 1);
      return false;
    }
    requiredLogin(() => $.post('/Social/Blog/CommentMicroBlog', { blogId: this.props.id, combody }, () => {
        ...
        <div className="interact_tabcont forward_detail" id={`commit-box-${this.props.id}`} style={{ display: 'none' }}>
          <div>
            ...
            <div className="right_interactbox">
              <textarea className="W_input" id={`comment_txt_${this.props.id}`} ref="commentInput" />
              <div className="p_opt clearfix">
                <ul className="rp_tool_list clearfix" style={{ position: 'relative' }}>
                  ...
                  <li data-id={this.props.id}>
                    <a style={{ cursor: 'pointer' }} data-id={`comment_txt_${this.props.id}`} className="p123" ><em className="weibo-icon i-face" />表情</a>
                  </li>
                  <li>
                    <span>
                        <a onClick={this.handleLoadUser.bind(this)} style={{ cursor: 'pointer' }} input-id={`comment_txt_${this.props.id}`} div-id={`comment_box_${this.props.id}`}>
                            <em className="weibo-icon i-user" />用户
                                            </a>
                      </span>
